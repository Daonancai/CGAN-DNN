...
for it in range(1000000):
    if it % 1000 == 1:
        ...
        y_sample_1[:, 0] = 1
        samples = sess.run(G_sample, feed_dict={Z: Z_sample, y:y_sample_1})

        fig = plot(samples)
        plt.savefig('out/{}.png'.format(('1_'+str(i)).zfill(3)), bbox_inches='tight')
        i += 1
        plt.close(fig)


    #X_mb, y_mb = mnist.train.next_batch(mb_size)
    y_mb, X_mb = dataset.nextBatch(mb_size)
